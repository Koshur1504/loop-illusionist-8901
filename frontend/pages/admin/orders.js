...
function pagination(TotalPages, limit, queryParams) {
  ...
  prevBtn.addEventListener("click", () => {
    if (currentPage > 1) {
      currentPage--;
      fetchData(
        `${ordersUrl}?_page=${currentPage}&_limit=${limit}`,
        queryParams,
      );
    }
  });
...
sortDropdown.addEventListener("change", () => {
  ...
  if (selectedOption === "newToOld") {
    fetchData(
      `${productsUrl}?_page=1&_limit=20&_sort=expected_delivery_date&_order=asc`,
    );
  } else if (selectedOption === "oldToNew") {
    fetchData(
      `${productsUrl}?_page=1&_limit=20&_sort=expected_delivery_date&_order=desc`,
    );
  } else {
